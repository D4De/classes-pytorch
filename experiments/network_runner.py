...
def yolo_detection_run(
    ...
    if use_single_batch:
        ...
        with torch.no_grad():
            golden_results = model.predict(imgs, imgsz=image_size, device=0, batch=len(imgs), verbose=False)

        # targets is a list of dictionaries, each describing a bounding box. The 'labels' entry (type Tensor) is used as the label
        ...
        with torch.no_grad():
            for imgs, target in tqdm(dataloader, desc='Performing golden run', colour='yellow'):
                ...
                for target_item in target:
                    target_item['labels'].apply_(lambda x: id_mapping[x])
                targets += list(target)
