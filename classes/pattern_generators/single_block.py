...
def single_block_generator(
    ...
    block_start_offset = np.random.randint(0, max(1, num_values_per_tensor - block_size))
    ...
    c_idxs, h_idxs, w_idxs = np.unravel_index(
        np.array([block_start_offset + idx for idx in block_corrupted_idxs if block_start_offset + idx < mask.size], dtype=np.intp), shape=(c, h, w)
    )
