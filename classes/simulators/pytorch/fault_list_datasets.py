...
class FaultListFromTarFile(Dataset[PyTorchFault]):
    ...
    def __getitem__(self, index : int) -> PyTorchFault:
        ...
        fault_file_path = os.path.join(selected_module_name, f"{file_idx}_faults_{selected_module_name}.npz")
        ...
        with tarfile.TarFile(self.fault_list_path, 'r') as tarf:
            ...
            try:
                ...
                return PyTorchFault(selected_module_name, index, torch_mask, torch_values, sp_class, sp_param)
            finally:
                member_file.close()
    ...
    def collate_fn(self, data : Sequence[PyTorchFaultBatch]):
        ...
        return PyTorchFaultBatch(names, fault_ids, batch_masks, batch_values, batch_values_idxs, sp_classes, sp_parameters)
...
class FaultListFromGenerator(IterableDataset):
    def __init__(self, fault_list_generator : PyTorchFaultList, module : str, n_faults : int, batch_size : int) -> None:
        super().__init__()
