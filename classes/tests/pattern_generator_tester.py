            ...
            try:
                ...
                if image_output_folder_path is not None:
                    image_path = os.path.join(
                        image_output_folder_path,
                        f"{test_name}_{sp_name}_{count}_{tr}.png",
                    )
                    plot_mask(
                        mask,
                        layout_type=layout,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--channel-sizes",
        nargs="+",
        help="List of channel sizes to evaluate, specified as space separated tuples of two items each representing channel height and width. Example: -d 11,11 22,22 40,40",
        default=[
            "1,1",
            "7,7",
            "16,16",
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="List of number of channels, specified as space separated integers. Example: -c 1 2 10 20",
        default=[1, 3, 4, 12, 32, 100, 128, 137, 256],
    )
    ...
    for i, ch in enumerate(args.channel_sizes):
        ...
        if len(members) != 2:
            parser.error(
                f"Element {ch} at position {i} could not be parsed in --channel-sizes could not be parsed. (Tuples of two comma separated integeres are required)"
            )
        a, b = members
        a, b = int(a), int(b)
        if a <= 0 or b <= 0:
            parser.error(
                f"An element of {ch} at position {i} is non-positive. All channel in --channel-sizes dimensions must be strictly positive."
            )
