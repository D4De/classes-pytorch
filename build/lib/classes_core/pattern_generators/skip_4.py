...
from .generator_utils import convert_to_linearized_index, random_channels, random_int_from_pct_range
...
def skip_4_generator(
    ...
    candidate_corrupt_positions = [
        (chan, (map_idx + starting_map_offset) * skip_amount + remainder)
        for chan in channels
        for map_idx in range(unique_map_indexes)
    ]
    candidate_corrupt_positions = convert_to_linearized_index(
        candidate_corrupt_positions, output_shape
    )
    number_of_corrupted_pos = random_int_from_pct_range(
        len(candidate_corrupt_positions), *params["indexes_corruption_pct"]
    )
    corrupt_positions = np.random.choice(
        candidate_corrupt_positions, number_of_corrupted_pos
    )
