...
class Simulator(nn.Module):
    def __init__(self, layer_type, size = None, models_folder = './models', fixed_spatial_class = None, fixed_domain_class = None, enable = True):
        super().__init__()
        self.sites_count = 1
    ...
    def forward(self, x):
        if not self.enable:
            return x
        injection_site = self.__generate_injection_sites(1, self.layer_type, self.size or str(tuple(x.shape)), self.models_folder)
...
def create_simulator_hook(torch_type_to_layer_model_map : Dict[str, str], layer_name : str, fixed_layer_type : Optional[str] = None, fixed_size : Optional[Iterable] = None, models_folder : str = './models', fixed_spatial_class : Optional[str] = None, fixed_domain_class : Optional[str] = None, data_capture_func : Optional[Callable[[ClassesSimulationData], None]] = None, verbose : bool = False):
    def _hook(module, input, output):
        ...
        layer_type = fixed_layer_type or torch_type_to_layer_model_map[type(module).__name__]
        injection_site = InjectableSite(layer_type, str(output_shape))
        
        if verbose:
            print('--- Injection details ---')

        injection_sites, (spatial_pattern, spatial_parameters, domain_pattern, corrupted_channel_count) = InjectionSitesGenerator([injection_site], models_folder, fixed_spatial_class = fixed_spatial_class, fixed_domain_class= fixed_domain_class) \
                .generate_random_injection_sites(1, return_pattern_details = True)
        
        if data_capture_func is not None:
            data_capture_func(ClassesSimulationData(
                layer_name,
