                ...
                for k in range(i_map + 1, len(mapList)):
                    if nErrorKey in mapList[k].keys():
                        if errorPatternKey in mapList[k][nErrorKey]["FF"]:
                            ...
                            probSum += float(weightsList[k]) * float(
                                mapList[k][nErrorKey]["FF"][errorPatternKey]
                            )

                if errorPatternKey not in finalSpatial[nErrorKey]["FF"]:
                    finalSpatial[nErrorKey]["FF"] = initializeKey(
                        finalSpatial[nErrorKey]["FF"], errorPatternKey
                    )
                finalSpatial[nErrorKey]["FF"][errorPatternKey] = float(probSum) / float(
                    nMapSharingKey
                )
                ...
                if key != "MAX":
                    ...
                    for k in range(i_map + 1, len(mapList)):
                        if key in mapList[k][nErrorKey]:
                            nMapSharingKey += float(weightsList[k])
                            probSum += float(weightsList[k]) * float(
                                mapList[k][nErrorKey][key]
                            )

                    if key not in finalSpatial[nErrorKey]["FF"]:
                        finalSpatial[nErrorKey]["PF"] = initializeKey(
                            finalSpatial[nErrorKey]["PF"], key
                        )

                    finalSpatial[nErrorKey]["PF"][key] = float(probSum) / float(
                        nMapSharingKey
                    )
                else:
                    ...
                    for k in range(i_map + 1, len(mapList)):
                        if "MAX" in mapList[k][nErrorKey]["PF"]:
                            if (
                                finalSpatial[nErrorKey]["PF"]["MAX"]
                                < mapList[k][nErrorKey]["PF"]["MAX"]
                            ):
                                finalSpatial[nErrorKey]["PF"]["MAX"] = mapList[k][
                                    nErrorKey
                                ]["PF"]["MAX"]
