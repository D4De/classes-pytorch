...
def main():
    ...
    test_images = np.random.choice(
        len(test_dataset), size=N_IMAGES, replace=False
    ).astype(int)
    images_results = {}
    exp_results = defaultdict(dict)

    prog_bar = tqdm(
        total=N_INJECTABLE_LAYERS * N_IMAGES * N_SIM_PER_IMAGE * len(SPATIAL_CLASSES)
    )
    ...
    for injectable_layer_id in range(N_INJECTABLE_LAYERS):
        for spatial_class in SPATIAL_CLASSES:
            error_simuator_net = LeNet5Simulator(
                N_CLASSES,
                selected_layer=injectable_layer_id,
                fixed_spatial_class=spatial_class,
            )
            with torch.no_grad():
                for name, _ in model.named_parameters():
                    layer_name = name.split(".")[0]
                    eval(
                        f"error_simuator_net.{layer_name}.weight.copy_(model.{layer_name}.weight)"
                    )
                    eval(
                        f"error_simuator_net.{layer_name}.bias.copy_(model.{layer_name}.bias)"
                    )
            ...
            exp_results[spatial_class][injectable_layer_id] = {"images": images}

    with open("sp_class_campaign.json", "w") as f:
        json.dump(exp_results, f, indent=2)

    accuracy = (
        sum(1 for v in images_results.values() if v["label"] == v["golden_prediction"])
        / N_IMAGES
    )
    ...
    for sp_class, sp_class_data in exp_results.items():
        for layer, layer_data in sp_class_data.items():
            for image_id, image_data in layer_data["images"].items():
                fault_tol_preds[sp_class] += image_data["results"][
                    image_data["golden_prediction"]
                ]
