...
class LeNet5Simulator(nn.Module):
    ...
    def __inject_error_if_requested(
        self,
        layer_num,
        operator_type,
        fixed_spatial_class=None,
            ...
            if self.selected_layer == layer_num:
                ...
                if len(layer_output_shape) < 4:
                    layer_output_shape = [1] * (
                        4 - len(layer_output_shape)
                    ) + layer_output_shape
                layer_output_shape = str(tuple(layer_output_shape))
                if verbose:
                    print(
                        f"Injecting after layer {self.selected_layer} ({operator_type}) with shape {layer_output_shape}"
                    )
                corrupted_layer_output = Simulator(
                    operator_type,
                    layer_output_shape,
                    MODELS_FOLDER,
                    fixed_spatial_class or self.fixed_spatial_class,
                    fixed_domain_class or self.fixed_domain_class,
                )(layer_output)
    ...
    def forward(self, x):
        ...
        x = self.__inject_error_if_requested(
            6, self.conv_strategy, fixed_spatial_class="bullet_wake"
        )(x)
        x = self.tanh3(x)
        x = self.__inject_error_if_requested(
            7, "tanh", fixed_spatial_class="bullet_wake"
        )(x)
        x = torch.flatten(x, 1)
        x = self.linear1(x)
        x = self.tanh4(x)
...
def load_datasets():
    ...
    train_dataset = datasets.MNIST(
        root="mnist_data", train=True, transform=transf, download=True
    )

    valid_dataset = datasets.MNIST(root="mnist_data", train=False, transform=transf)
...
def main():
    ...
    model_simulator = LeNet5Simulator(
        N_CLASSES, 4, fixed_domain_class={"out_of_range": [100.0, 100.0]}
    ).to("cpu")
    ...
    print(
        f"Predctions not changed w.r.t Vanilla model: {fault_robust_runs} of {N_IMAGES * RUN_PER_IMAGE}"
    )
