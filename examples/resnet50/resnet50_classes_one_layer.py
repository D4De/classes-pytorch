"""
Example run of CLASSES on a trained ResNet50 using the GTSRB dataset.
A few faults are generated for the first layer of the network.
"""
import torch
import torchvision
...
# REMEMBER TO RUN FROM CLASSES ROOT FOLDER
import sys
...
from experiment_utils import find_corrupted_output_rows, find_ranking_shifts
...
def run_inference(
    ...
    with torch.no_grad():
        scores = torch.zeros((sample_count, num_classes), device=device)
        labels = torch.zeros((sample_count), device=device, dtype=torch.long)
    
        for batch_id, (image, label) in enumerate(pbar):
...
if __name__ == '__main__':
    example_folder_path = sys.path[0]
    
    data_path = example_folder_path + '/data'
    assert os.path.exists(data_path), "Dataset directory not found, check path"
    
    trained_model_path = example_folder_path + '/checkpoints/resnet50_gtsrb_best.pth'
    ...
    # now use CLASSES to perform error simulation
    error_model_folder = './error_models/models'
    
    conv_error_model = ErrorModel.from_model_folder(error_model_folder, 'conv_gemm')
    ...
    generator = FaultGenerator(conv_error_model)
