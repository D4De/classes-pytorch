...
def build_model_with_simulator(
    input_shape, available_injection_sites, masks, num_inj_sites
):
    inputs = keras.Input(shape=input_shape, name="input")
    conv1 = layers.Conv2D(
        filters=6, kernel_size=(5, 5), activation="relu", name="conv1"
    ...
    pool2 = layers.MaxPool2D(pool_size=(2, 2), strides=(1, 1), name="maxpool2")(
        simulator
    )
    conv3 = layers.Conv2D(
        filters=120,
...
model_with_simulator = build_model_with_simulator(
    x_train[0].shape, available_injection_sites, masks, len(available_injection_sites)
)
...
for layer_name in model_base_layers:
    model_with_simulator.get_layer(layer_name).set_weights(
