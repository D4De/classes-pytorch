...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Convolutional neural network on CIFAR-10 dataset."
    )
    parser.add_argument(
        "-m",
        "--mode",
...
def load_cifar10_dataset():
    ...
    for channel in range(0, 3):
        x_train[:, channel, :, :] = (
            x_train[:, channel, :, :] - cifar10_means[channel]
        ) / cifar10_stds[channel]
        x_test[:, channel, :, :] = (
            x_test[:, channel, :, :] - cifar10_means[channel]
        ) / cifar10_stds[channel]
    validation_percentage = 0.3
    validation_size = np.floor(float(x_train.shape[0]) * validation_percentage).astype(
        "int"
    )
    validation_indexes = np.random.choice(
        x_train.shape[0], validation_size, replace=False
    )
...
def build_model():
    ...
    with tf.variable_scope("maxpool_1"):
        max_pool_1 = tf.layers.MaxPooling2D(
    ...
    with tf.variable_scope("maxpool_2"):
        max_pool_2 = tf.layers.MaxPooling2D(
            2, 2, padding="same", data_format="channels_first"
        )(batch_norm_4)
    with tf.variable_scope("dropout_2"):
        dropout_2 = tf.layers.Dropout(rate=0.0)(max_pool_2)
    ...
    with tf.variable_scope("maxpool_3"):
        max_pool_3 = tf.layers.MaxPooling2D(
            2, 2, padding="same", data_format="channels_first"
        )(batch_norm_6)
    ...
    with tf.variable_scope("dense_1"):
        dense_1 = tf.layers.Dense(
            576, activation=tf.nn.relu, kernel_initializer=tf.glorot_uniform_initializer
        )(flatten)
    with tf.variable_scope("dropout_4"):
        dropout_4 = tf.layers.Dropout(rate=0.5)(dense_1)
    with tf.variable_scope("dense_2"):
        logits = tf.layers.Dense(10, kernel_initializer=tf.glorot_uniform_initializer)(
            dropout_4
        )
        ...
        for step in range(steps + 1):
            ...
            if step % 100 == 0:
                loss, accuracy = session.run(
                    [loss_operator, accuracy_operator],
                    feed_dict={model[0]: x_train_batch, model[1]: y_train_batch},
                )
                validation_batches = np.ceil(
                    float(dataset[4].shape[0]) / float(batch_size)
                ).astype("int")
                total_correct = 0
                total_loss = 0.0
                for validation_batch in range(validation_batches):
                    ...
                    batch_loss, batch_correct = session.run(
                        [loss_operator, accuracy_batch],
                        feed_dict={
                            model[0]: x_validation_batch,
                ...
                if (
                    ...
                    print("Model saved.")
                print(
                    "Step: {:0>4d}\tTr. loss: {:.5f}\tTr. acc: {:.4f}\t\tVal loss: {:.5f}\tVal acc: {:.4f}".format(
                        step, loss, accuracy, validation_loss, validation_accuracy
                    )
                )
...
def test(dataset, model):
    instances = [
        ...
        (
            "FusedBatchNorm",
            ("batchnorm_2/batch_normalization/FusedBatchNorm", "(?, 32, 32, 32)"),
        ),
        (
            "FusedBatchNorm",
            ("batchnorm_6/batch_normalization/FusedBatchNorm", "(?, 128, 8, 8)"),
        ),
        (
            "FusedBatchNorm",
            ("batchnorm_5/batch_normalization/FusedBatchNorm", "(?, 128, 8, 8)"),
        ),
        (
            "FusedBatchNorm",
            ("batchnorm_4/batch_normalization/FusedBatchNorm", "(?, 64, 16, 16)"),
        ),
        (
            "FusedBatchNorm",
            ("batchnorm_1/batch_normalization/FusedBatchNorm", "(?, 32, 32, 32)"),
        ),
        (
            "FusedBatchNorm",
            ("batchnorm_3/batch_normalization/FusedBatchNorm", "(?, 64, 16, 16)"),
        ),
    ]
            ...
            for golden_digit in indexes.keys():
                for index in indexes[golden_digit]:
                    ...
                    for _ in range(repetitions):
                        # while True:
                        res = error_sim.generate_injection_sites(
                            "OPERATOR_SPECIFIC", 1, instance[0], op_instance=instance[1]
                        )

                        if not res:
                            continue

                        results, _, _ = error_sim.inject([model[3]], feed_dict)

                        correct_result = golden_digit == np.argmax(results[0][0])
                        if not correct_result:
                            errors += 1
            print(
                f"Instance {instance[1][0]} got {str(errors)} misclassifications over {str(repetitions)} simulations"
            )
